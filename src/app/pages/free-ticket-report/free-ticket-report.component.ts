...
@Component({
  selector: 'app-free-ticket-report',
  templateUrl: './free-ticket-report.component.html',
  styleUrls: ['./free-ticket-report.component.css']
})
export class FreeTicketReportComponent implements OnInit {
  ...
  competition:any;
  freeticketsdata = [];
  freeticketcompetitionarray = [];
  emptyfreeticketsdata:boolean =true;
  ...
  constructor(private Activate: ActivatedRoute,private fb:FormBuilder,private location: Location,private router: Router,private ds: DataserviceService,private toastr: ToastrService) {
    ...
    this.getfreeticket_competition_data();
    this.dtOptions = {
  ...
  ngOnInit(): void {
    this.freeticketsform = this.fb.group({
  ...
  get formControls() { return this.freeticketsform.controls; }

  getcompetitons_data(){
    this.ds.getmethod('competitions',this.logininfo['success']['token']).subscribe(res => {
...
// this.competitionarray =competitionarray;
  ...
  getfreeticket_competition_data(){
    this.isloading = true;
    this.emptyfreeticketsdata = false;
    this.ds.getmethod('freeticketreports',this.logininfo['success']['token']).subscribe(res => {
      this.isloading = false;
      if(res['status'] == 'SUCCESS'){
        let data = res['data'];
        console.log(data);
        if(data == ''){
this.emptyfreeticketsdata = true;
        }else{
        data.forEach(f => {
          this.freeticketcompetitionarray.push({competition_id:f.competition_id,competition:f.competition,draw_end_date_time:f.draw_end_date_time,draw_start_date_time:f.draw_start_date_time,
            no_of_tickets:f.no_of_tickets,ticket_count:f.ticket_count,ticket_price:f.ticket_price});
          
        })
        if(this.freeticketcompetitionarray.length <= 0){
this.emptyfreeticketsdata = false;
        }
        this.dtTrigger.next();
      }
// this.competitionarray =competitionarray;
  ...
  generatePdf(action = 'open') {
    ...
    if(this.freeticketsform.invalid){
      return;
    }else{
      this.ds.getmethod('report/'+this.freeticketsform.value.competition,this.logininfo['success']['token']).subscribe(res => {
        if(res['status'] == "SUCCESS"){
let data = res['data'];
...
let start_date= this.freeticketsform.value.start_date;
let end_date= this.freeticketsform.value.end_date;
...
viewfreetickets(data){
  const navigationExtras = {
    queryParams: {
      competition_id: data.competition_id  
    }
};
this.router.navigate(['/view-freeticketusers'], navigationExtras);
}
